...
//web3.setProvider(new web3.providers.HttpProvider('http://localhost:8079'));
...
var Steadlly = Steadlly || {
        ...
        /***
         * Returns the skills from an user
         * @returns {Array|*}
         */
        ...
        account: {
            ...
            _company: false,
            /***
             * It selects an user mode
             * @param {string} option
             * @param {bool} toggleUI - true if the ui should be toggled
             */
            useAs : function(option, toggleUI){
                if(option === "company") {
                    if (this.verifyCompanyOwner()) {
                ...
                if(toggleUI){
                    if(Steadlly.ui != undefined){
                        Steadlly.ui.toggleAccountType();//if(localStorage['accountType'])
                    }
                }
            },
        ...
        _loadModules: function (modules) {
            if (modules.indexOf('ui') != -1) {
                Steadlly.ui = {
                    ...
                    /***
                     * Toggles the account button on top
                     */
                    toggleAccountType : function(){
                        if(localStorage['accountType'] == "company") {
